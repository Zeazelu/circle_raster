...
ActiveDelegate::ActiveDelegate(QObject *parent)
    : QAbstractItemDelegate(parent),
      m_fieldSize(FIELD_SIZE)
{
}

void ActiveDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
...
QSize ActiveDelegate::sizeHint(const QStyleOptionViewItem & /* option */,
                              const QModelIndex & /* index */) const
{
    return QSize(m_fieldSize, m_fieldSize);
}

void ActiveDelegate::getStyleForField(const SField& field, QStyleOptionButton& buttonStyle) const
{
    if (field.discovered)
    {
...
void ActiveDelegate::getGeneralsForField(const SField& field, const QStyleOptionViewItem &option,
